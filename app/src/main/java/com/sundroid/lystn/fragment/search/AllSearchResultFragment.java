package com.sundroid.lystn.fragment.search;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.recyclerview.widget.DefaultItemAnimator;
import androidx.recyclerview.widget.LinearLayoutManager;
import androidx.recyclerview.widget.RecyclerView;

import com.sundroid.lystn.R;
import com.sundroid.lystn.adapter.SearchResultAdapter;
import com.sundroid.lystn.fragmentcontroller.FragmentController;

import java.util.ArrayList;
import java.util.List;

import butterknife.BindView;

public class AllSearchResultFragment extends FragmentController {

    @BindView(R.id.rv_result_1)
    RecyclerView rv_result_1;
    @BindView(R.id.rv_podcasts)
    RecyclerView rv_podcasts;
    @BindView(R.id.rv_episodes)
    RecyclerView rv_episodes;
    @BindView(R.id.rv_artists)
    RecyclerView rv_artists;
    @BindView(R.id.rv_downloads)
    RecyclerView rv_downloads;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view=inflater.inflate(R.layout.frag_all_search,container,false);
        setUpView(getActivity(),this,view);
        return view;
    }

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);

        searchResultPOJOS.add("");
        searchResultPOJOS.add("");
        searchResultPOJOS.add("");
        searchResultPOJOS.add("");

        attachAdapter();
        attachGenericAdapter(rv_artists);
        attachGenericAdapter(rv_podcasts);
        attachGenericAdapter(rv_episodes);
        attachGenericAdapter(rv_downloads);

    }


    SearchResultAdapter searchResultAdapter;
    List<String> searchResultPOJOS = new ArrayList<>();

    public void attachAdapter() {
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false);
        rv_result_1.setHasFixedSize(true);
        rv_result_1.setLayoutManager(linearLayoutManager);
        searchResultAdapter = new SearchResultAdapter(getActivity(), this, searchResultPOJOS);
        rv_result_1.setAdapter(searchResultAdapter);
        rv_result_1.setNestedScrollingEnabled(false);
        rv_result_1.setItemAnimator(new DefaultItemAnimator());
    }



    public void attachGenericAdapter(RecyclerView rv) {
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false);
        rv.setHasFixedSize(true);
        rv.setLayoutManager(linearLayoutManager);
        SearchResultAdapter searchResultAdapter = new SearchResultAdapter(getActivity(), this, searchResultPOJOS);
        rv.setAdapter(searchResultAdapter);
        rv.setNestedScrollingEnabled(false);
        rv.setItemAnimator(new DefaultItemAnimator());
    }
}
